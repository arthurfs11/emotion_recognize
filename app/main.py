...
# # app/main.py (substitua suas funções por estas)
# import subprocess, shutil

# def _abrir_camera_configurada():
#     # Tenta abrir com backends/índices diferentes (macOS)
#     candidates = []
#     try:
#         import cv2
#         candidates = [
#             (0, cv2.CAP_AVFOUNDATION),  # backend nativo do mac
...
#                 if cap: cap.release()
#                 continue
#             # Configurações básicas (se suportado)
#             try:
...
# def _capturar_por_imagesnap(pasta: str) -> str | None:
#     """
#     Fallback usando 'imagesnap' (brew install imagesnap).
#     """
#     if not shutil.which("imagesnap"):
#         return None
#     path = os.path.join(pasta, f"captura_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
#     try:
#         # -w 1 = espera 1s (warmup da câmera)
#         subprocess.check_call(["imagesnap", "-w", "1", path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
#     # 1) Tenta via OpenCV com retries/backends/índices
...
#     # 2) Fallback: imagesnap (captura única)
#     path = _capturar_por_imagesnap(pasta)
#     if path:
#         return path

#     # 3) Se nada deu certo, falha controlada
#     raise RuntimeError("Falha de I/O da câmera (sem acesso/dispositivo em uso/permissão negada)")
...
#         # Persistência
#         if emocoes is None:
#             # fallback consistente com seu domínio
#             emocoes = {
#                 'raiva': 0.0, 'desgosto': 0.0, 'medo': 0.0,
#                 'feliz': 0.0, 'triste': 0.0, 'surpresa': 0.0, 'neutro': 1.0
#             }
#             # Você também pode marcar o dominante como 'usuario_ausente' via meta:
#             meta.setdefault("emocao_dominante_forcada", "usuario_ausente")

#         # Persistência
#         salvar_em_banco(emocoes, recursos, data_captura, pessoa_id, meta)
#         resultado = registrar_amostra(emocoes, recursos, data_captura, pessoa_id, meta=meta, user_id="default")
...
# app/main.py (corrigido)
# - Ajusta sys.path para rodar com `python3 app/main.py`
# - Unifica funções de câmera (sem duplicidades) com backends/índices e fallback imagesnap
# - Protege pipeline quando emocoes=None
# - Mantém economia agressiva e metadados
...
# Garantir que a raiz do projeto esteja no path (permite `from services...` e `from config...`)
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))
...
from config.log import *
from services.telemetria import *  # registrar_amostra
...
def _abrir_camera_configurada():
    """Tenta abrir a câmera com diferentes backends/índices (macOS-friendly)."""
    candidates = [
...
def _capturar_por_imagesnap(pasta: str) -> Optional[str]:
    """Fallback usando 'imagesnap' (brew install imagesnap)."""
    if not shutil.which("imagesnap"):
        return None
    path = os.path.join(pasta, f"captura_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
...
def _normalizar_emocoes_pt(emocoes):
    ...
    out = {}
    for k, v in emocoes.items():
        k2 = _MAP_EN_PT.get(str(k).lower(), str(k).lower())
        out[k2] = float(v) if v is not None else 0.0
    for k in ["feliz","triste","medo","raiva","desgosto","surpresa","neutro"]:
        out.setdefault(k, 0.0)
    return out
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning(f"[BOOTSTRAP] Falha ao capturar imagem inicial: {e}")
        img_bootstrap = None
    ...
    if emb_now is not None:
        try:
            ...
            logger.warning(f"[IDENTIDADE] Falha ao salvar embedding: {e}")
    try:
        if img_bootstrap and os.path.exists(img_bootstrap):
            os.remove(img_bootstrap)
    except Exception:
    ...
    while True:
        ...
        if ECONOMIA_ATIVA:
            ...
            if cpu_alta or em_cooldown or ruim_streak >= RUIM_STREAK_LIMIAR:
                ...
                try:
                    ...
                    meta["camera_status"] = meta.get("camera_status") or "erro"
        else:
        ...
        try:
            ...
            logger.error(f"[ALERTS] Falha ao registrar amostra: {e}")
